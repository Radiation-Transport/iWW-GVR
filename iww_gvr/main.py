...
class ww_item:
    ...
    def __init__(self, filename, X, Y, Z, nbins, No_Particle, ww1, eb1, ww2, eb2, dict):
        ...
        self.dim = [(self.X[-1] - self.X[0]) / (len(self.X) - 1), (self.Y[-1] - self.Y[0]) / (len(self.Y) - 1),
    ...
    def info(self):
        ...
        print(
            '{:>10}'.format('') + '\t' + Part_A.center(15, "-") + '\t' + Part_B.center(15, "-") + '\t' + Part_C.center(
                15, "-"))

        line_X = '{:>10}'.format('X -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Xo'])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Xf'])) + '\t' + '{:^15}'.format(len(self.X) - 1)
        line_Y = '{:>10}'.format('Y -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Yo'])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Yf'])) + '\t' + '{:^15}'.format(len(self.Y) - 1)
        line_Z = '{:>10}'.format('Z -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Zo'])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.d['B2_Zf'])) + '\t' + '{:^15}'.format(len(self.Z) - 1)
                ...
                if (L_COUNTER == 0):
                    ...
                    if split[-1] == 1.0000:
                        ...
                        for i in range(1, len(vec_coarse[0])):
                            X = np.concatenate(
                                (X, np.linspace(X[-1], vec_coarse[0][i], int(vec_fine[0][i - 1] + 1))[1:]))
                        B2_Xf = X[-1]

                        Y = [vec_coarse[1][0]]
                        for i in range(1, len(vec_coarse[1])):
                            Y = np.concatenate(
                                (Y, np.linspace(Y[-1], vec_coarse[1][i], int(vec_fine[1][i - 1] + 1))[1:]))
                        B2_Yf = Y[-1]

                        Z = [vec_coarse[2][0]]
                        for i in range(1, len(vec_coarse[2])):
                            Z = np.concatenate(
                                (Z, np.linspace(Z[-1], vec_coarse[2][i], int(vec_fine[2][i - 1] + 1))[1:]))
                        B2_Zf = Z[-1]
...
def analyse(self, zoneID, factor):
    ...
    for p in range(0, self.par):
        ...
        for e in range(0, len(self.eb[p])):
            ...
            posBins.append(
                len(np.argwhere(self.wwme[p][e] * zoneID > 0)) / (len(self.wwme[p][e]) * len(np.argwhere(zoneID > 0))))
            ...
            for k in range(1, (int(len(self.Z)))):
                for j in range(1, (int(len(self.Y)))):
                    for i in range(1, (int(len(self.X)))):
                        if (extM[k, j, i] > 0):
                            self.ratio[p][e][k - 1, j - 1, i - 1] = max(
                                [extM[k + 1, j, i], extM[k - 1, j, i], extM[k, j + 1, i], extM[k, j - 1, i],
                                 extM[k, j, i + 1], extM[k, j, i - 1]]) / extM[k, j, i]
                bar.update()
                # RATIO=[]
            bar.close()
    ...
    for e in range(0, len(self.ratio[p])):
        ...
        for i in range(0, len(x_axis) - 1):
            y_axis.append(
                len(np.where(np.logical_and(self.ratio[p][e] >= x_axis[i], self.ratio[p][e] < x_axis[i + 1]))[0]))
        ...
        fig.savefig(
            self.name + '_ParNo.' + str(p + 1) + '_' + 'E' + '=' + str(self.eb[p][e]) + 'MeV' + '_Ratio_Analysis.jpg')
...
def zoneDEF(self, degree):
    if degree == 'all':  # The ww is completely contained in the model domain
        ...
        non_zero_index = [i[3:] for i in
                          np.argwhere(self.wwme)]  # creates a list of indices of j,i if there is a non-zero value
        ...
        if flag:
            ...
            if flag:
                ...
                if vmin > 0:
                    ...
                    if vmin > 0:
                        cax = mpl.pyplot.imshow(vals, cmap=plt.get_cmap('jet', nColors),
                                                norm=colors.LogNorm(vmin, vmax), extent=extent)
                    else:
                        cax = mpl.pyplot.imshow(vals, cmap=plt.get_cmap('jet', nColors), vmin=vmin, vmax=vmax,
                                                extent=extent)
                    ...
                    if PAR_Select == 0 and self.d['B2_par'] == True:
                        fig.savefig(self.name + str(PAR_Select + 1) + '_' + PLANE + '=' + str(
                            PLANE_QUOTE) + 'cm' + '_' + 'E' + '=' + PE_STR + 'MeV' + '_ParNo.' + '.jpg')
                    else:
                        fig.savefig(self.name + str(PAR_Select) + '_' + PLANE + '=' + str(
                            PLANE_QUOTE) + 'cm' + '_' + 'E' + '=' + PE_STR + 'MeV' + '.jpg')
                        # problem here -->mpl.pyplot.show(block = False)
                    print(' Plot...Done!\n')
...
def gvr_soft(gvrname):
    ...
    if not m.cart:
        ...
        vec_coarse = [X, Y, Z]
        vec_fine = [[1 for i in range(len(vec_coarse[0]) - 1)], [1 for i in range(len(vec_coarse[1]) - 1)],
        ...
        d = {'B1_if': 1,
             ...
             'vec_fine': vec_fine}
        gvr = ww_item(gvrname, X, Y, Z, nbins, nPar, ww1, eb, [], [],
                      d)  # A ww skeleton is generated with the info from the mesh file
    gvr.info()
    ...
    if gvr.coord == 'cart':
        ...
        for k in range(0, len(gvr.Z) - 1):
            for j in range(0, len(gvr.Y) - 1):
                for i in range(0, len(gvr.X) - 1):
                    ww_inp[k, j, i] = np.power(gvr.wwme[0][0][k, j, i] / fluxinp_max * (2 / (beta + 1)),
                                               soft)  # Van Vick/A.Davis
            bar.update()
        bar.close()
        ...
        if len(zoneID) > 0:  # Hole filling (Super efficient but the values are not averaged or interpolated)
            ...
            holes = holes == 1
            ww_inp = fill(ww_inp,
                          holes)  # The hole filling gives to the holes the same value as the nearest non-zero value
            ...
            while len(np.argwhere(ww.ratio[p][e] >= maxratio)) > 0:
                ...
                for idx in idxs:
                    ...
                    ww.wwme[p][e][tuple(idx)] = (max(neig)) / (
                                maxratio * 0.9)  # Reduce the ww value to one right below the maxim ratio allowed
        ...
        for line in infile:
            if BLOCK_NO == 1:
                ...
                if L_COUNTER == 2:
                    ...
                    X = [vec_coarse[0][0]]
                    for i in range(1, len(vec_coarse[0])):
                        X = np.concatenate((X, np.linspace(X[-1], vec_coarse[0][i], int(vec_fine[0][i - 1] + 1))[1:]))
                    B2_Xf = X[-1]

                    Y = [vec_coarse[1][0]]
                    for i in range(1, len(vec_coarse[1])):
                        Y = np.concatenate((Y, np.linspace(Y[-1], vec_coarse[1][i], int(vec_fine[1][i - 1] + 1))[1:]))
                    B2_Yf = Y[-1]

                    Z = [vec_coarse[2][0]]
                    for i in range(1, len(vec_coarse[2])):
                        Z = np.concatenate((Z, np.linspace(Z[-1], vec_coarse[2][i], int(vec_fine[2][i - 1] + 1))[1:]))
                    B2_Zf = Z[-1]
...
class ww_item_cyl:
    ...
    def __init__(self, filename, I, J, K, nbins, ww, B3_eb1, B3_eb2, dict):
        ...
        if len(self.eb1) == 1:
            ...
            self.wwme = np.array(ww[0]).reshape(len(self.eb1), self.d['B2_Kints'], self.d['B2_Jints'],
                                                self.d['B2_Iints'])

        if len(self.eb2) == 1:
            self.par = 2
            self.wwme = [self.wwme, np.array(
                [np.array(ww[1]).reshape(self.d['B2_Kints'], self.d['B2_Jints'], self.d['B2_Iints'])])]
        elif len(self.eb2) > 1:
            self.par = 2
            self.wwme = [self.wwme, np.array(ww[1]).reshape(len(self.eb2), self.d['B2_Kints'], self.d['B2_Jints'],
                                                            self.d['B2_Iints'])]
        else:
            self.wwme = [self.wwme, []]
            self.par = 1
    ...
    def info(self):
        ...
        line_X = '{:>10}'.format(' Radius -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.I[0])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.I[-1])) + '\t' + '{:^15}'.format(len(self.I) - 1)
        line_Y = '{:>10}'.format(' Height -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.J[0])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.J[-1])) + '\t' + '{:^15}'.format(len(self.J) - 1)
        line_Z = '{:>10}'.format(' Theta  -->') + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.K[0])) + '\t' + '{:^15}'.format(
            '{:8.2f}'.format(self.K[-1])) + '\t' + '{:^15}'.format(len(self.K) - 1)

        print(line_X)
        print(line_Y)
        print(line_Z)
        print('\n The mesh coordinates are cylindrical.')
        print('\n The file contain {0} particle/s and {1} voxels!'.format(len(self.d['B1_ne']),
                                                                          int(self.bins) * len(self.d['B1_ne'])))
            ...
            for j in Y:
                ...
                for i in X:
                    if self.coord == 'cart':  # The distance in calculated differently depending if the coordinates are cart or cyl
                        ...
                        dist = ((point[0] - focus[0]) ** 2 + (point[1] - focus[1]) ** 2 + (
                                    point[2] - focus[2]) ** 2) ** 0.5
                    I_vec.append(dist)
                J_vec.append(I_vec)
...
def zoneDEF_cyl(self, option, flag=False):  # Set flag to True to produce a vtk file with the zoneID
    ...
    if option == 'auto':
        ...
        for p in range(len(self.d['B1_ne'])):
            nz = [(i[1], i[3]) for i in
                  np.argwhere(self.wwme[p])]  # creates a list of indices of j,i if there is a non-zero value
...
def analyse_cyl(self, zoneID):
    ...
    for p in range(len(self.d['B1_ne'])):
                ...
                for j in range(1, len(self.J)):
                    for i in range(1, len(self.I)):
                        if (extM[k, j, i] > 0):
                            neig = [x for x in (
                            [extM[k + 1, j, i], extM[k - 1, j, i], extM[k, j + 1, i], extM[k, j - 1, i],
                             extM[k, j, i + 1], extM[k, j, i - 1]]) if x > 0]
                            if len(neig) > 0:
        ...
        for e in range(int(self.d['B1_ne'][p])):
            ...
            plt.xlabel('max ratio with nearby cells', fontdict=font)
            plt.ylabel('No.bins', fontdict=font)
            plt.title(self.name + '_ParNo.' + str(p + 1) + '_' + 'E' + '=' + str(self.eb12[p][e]) + 'MeV',
                      fontdict=font)
            # Tweak spacing to prevent clipping of ylabel
            plt.subplots_adjust(left=0.15)
            fig.savefig(self.name + '_ParNo.' + str(p + 1) + '_' + 'E' + '=' + str(
                self.eb12[p][e]) + 'MeV' + '_Ratio_Analysis.jpg')
...
def gvr_soft_cyl(m, mesh, gvrname):
    ...
    I = m.dims[3]
    vec_coarse = [I, J,
                  K]  # We build the vec_coarse and fine as if there are no fine meshes, all meshes are coarse with only 1 interval
    vec_fine = [[1 for i in range(len(vec_coarse[0]) - 1)], [1 for i in range(len(vec_coarse[1]) - 1)],
                [1 for i in range(len(vec_coarse[2]) - 1)]]
    ...
    dict = {'B1_if': 1,
            ...
            'vec_coarse': vec_coarse,
            'vec_fine': vec_fine}
    gvr = ww_item_cyl(gvrname, I, J, K, nbins, ww, B3_eb1, B3_eb2,
                      dict)  # A ww skeleton is generated with the info from the mesh file
    return gvr
