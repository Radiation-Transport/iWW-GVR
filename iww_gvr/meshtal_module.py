...
def splitn(string, n):
    """
    Return a list of n characters portions of a string
    """
            ...
            for ie in range(self.ldims[0]):
                ...
                for ix1 in range(rshape[0]):
                    ...
                    for ix0 in range(rshape[1]):
                        ...
                        )  # use dfloat conversion in case of no standar fortan exponent
                    skipLines(f, 3)
                    for ix0 in range(rshape[1]):
...
def diffmesh(m1, m2, absvalue=False, relative=False):
    ...
    msum.err = np.divide(
        msum.dat, denominator, out=np.zeros_like(msum.dat), where=(denominator != 0)
    )
...
def addbin(m1, binlist, flist=[], corr=False):
    ...
    if not corr:
        ...
        msum.err = np.divide(
            msum.err, msum.dat, out=np.zeros_like(msum.err), where=(msum.dat != 0)
        )
