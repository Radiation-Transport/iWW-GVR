...
# TODO alvaro: Implement tests for all the functions and properties use cases
...
class TestWW(unittest.TestCase):
    ...
    def test_properties_complex_cart(self):
        ...
        np.testing.assert_array_almost_equal(
            np.array([-15.0, -4.95, 5.1, 8.65, 12.2, 12.75, 13.3]), ww.vector_i
        )
        expected_values = {
            "n": {
                1.1: np.array(
...
"""
    ...
    def test_vector_i(self):
        ww = WW.read_from_ww_file(find_data_file("data/ww_complex_cart"))
        expected_vector = np.array([-15.0, -4.95, 5.1, 8.65, 12.2, 12.75, 13.3])
    ...
    def test_calculate_ratios(self):
        ...
        for particle in actual_ratio.keys():
            for energy in actual_ratio[particle].keys():
                np.testing.assert_array_almost_equal(
                    expected_ratio[particle][energy], actual_ratio[particle][energy]
                )
